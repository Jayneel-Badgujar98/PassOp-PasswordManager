...
const Manager = () => {
    ...
    const deletePassword = (id) => {
        ...
        let confirmation = confirm(`Are you sure want to delete this password ?, This password will be deleted Permanently!`);
        if (confirmation) {
    ...
    const editPassword = (id) => {
        let confirmation = confirm(`Are you sure want to edit this password ?, This password will be Edited Permanently!`);
        if (confirmation) {
            ...
            <div className=" mx-auto lp:bg-slate-800 p-4 mt-5 lp:px-40 lp:py-16  lp:w-[70%] font-semibold lp:mt-[1%] lp:mb-[1%] md:flex lp:justify-center lp:items-center">
                ...
                <div className=' text-white flex flex-col md:flex-row p-4 gap-4  w-full md:w-[50%]'>
                    <input
                        ...
                        className="rounded-full md:w-full w-[90%] mx-auto border-2 px-5 py-2.5 mt-2 border-[#00ff00] outline-[#000] flex justify-start items-center lp:py-2 lp:px-5 lp:mt-6 text-black"
                        type="text"
                        name="site"
                    />
                    ...
                    <div className='flex lp:flex-row flex-col justify-evenly gap-4 lp:gap-16 mb-6'>
                        <div className='w-[100%] lp:w-[60%]'>
                            <input
                                ...
                                className="rounded-full  w-[90%] mx-auto border-2 px-5 py-2.5 mt-2 border-[#00ff00] outline-[#000] flex justify-start items-center lp:py-2 lp:px-5 lp:mt-2 lp:mx-auto  text-black"
                                type="text"
                                name='username'
                                id='username'
                            />
                        ...
                        <div className='relative '>
                            <input
                                ...
                                className="rounded-full relative lp:relative w-[90%] mx-auto border-2 px-5 py-2.5 mt-2 border-[#00ff00] outline-[#000] flex justify-start items-center lp:py-2 lp:px-5 lp:mt-2 lp:mx-auto  text-black"
                                type={passwordVisibility === 'visibility' ? 'password' : 'text'}
                                name='password'
                                id='password'
                            />

                            <span className='absolute lp:absolute top-3.5 right-3 lp:top-3.5 lp:right-0 lp:py-1 lp:px-6 text-[#000] items-center py-2 px-9'>
                       ...
                       <tbody className="bg-green-300 text-black">
                         {passwordArray.map((item, index) => (
                           <tr key={index} className="border-b-2 border-black">
                             <td className="whitespace-normal break-normal break-all py-4 text-sm lp:text-lg text-center relative border-r-2 border-black border-b-3 rounded-bl-md">
                               <a href={item.site} target="_blank" rel="noopener noreferrer">
                                 {item.site}
                               </a>
                               <div className="flex justify-center tb:absolute mt-2 cursor-pointer tb:top-2 top-7 lp:top-3 tb:right-3" onClick={() => copyText(item.site)}>
                                 <div className="relative inline-block">
                                   <div className="absolute whitespace-nowrap bottom-[-10px] left-1/2 transform -translate-x-1/2 text-sm opacity-100 transition-opacity duration-300 hover:opacity-100">
                                     Copy
                                   </div>
                                   <lord-icon
                             ...
                             <td className="whitespace-normal break-normal break-all py-4 text-sm lp:text-lg text-center relative border-r-2 border-black border-b-3">
                               {item.username}
                             ...
                             <td className="py-3 lp:py-4 text-sm lp:text-lg text-center relative flex flex-col gap-3 tb:flex-row justify-center border-r-2 border-black border-b-3 rounded-br-md">
                               <span className="relative mx-auto " onClick={() => { editPassword(item.id); }}>
                                 <lord-icon
                                   ...
                                   trigger="hover"
                                 ></lord-icon>
                                 <div className="absolute bottom-[-10px] left-1/2 transform -translate-x-1/2 text-sm opacity-100 transition-opacity duration-300 hover:opacity-100">
                                   Edit
                                 </div>
                               </span>
